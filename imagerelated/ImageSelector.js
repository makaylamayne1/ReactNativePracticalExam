...
import { View, Button, StyleSheet, Alert, CameraRoll ,TextInput, Image, Text,TouchableOpacity,Title} from 'react-native';
...
const ImageSelector = props => {
    ...
    //image uri 
    const [selectedImage, setSelectedImage] = useState('https://getwallpapers.com/wallpaper/full/a/e/9/825536-download-purple-wallpaper-desktop-1920x1200.jpg');
  ...
  const [textToNote, setTextToNote] = useState('');
    ...
    useEffect(() => {
        db.transaction(tx => {
          tx.executeSql('CREATE TABLE IF NOT EXISTS pictureTable  (id INTEGER PRIMARY KEY NOT NULL, imageName TEXT, imageUrl TEXT);', 
            [], 
      ...
      saveToDatabase = async() => {
        ...
        db.transaction(
          tx => {
            ...
            tx.executeSql("INSERT INTO pictureTable (imageName, imageUrl) values (?, ?)", 
              [text1, selectedImage],
  ...
  retrieveFromDatabase = async() => {
    db.transaction(
      tx => {
        tx.executeSql("SELECT * FROM pictureTable", 
          [], 
          (_, { rows }) => {     
            ...
            entries.forEach((entry) => {
              setDataFromDatabase(prev => prev + `${entry.imageName}, ${entry.imageUrl}\n`);
    ...
    return (
        <View style={styles.container}>
            <View style={styles.buttonContainer}>
               ...
               <Button style={styles.button} title="Retrieve data from the database" onPress={()=>{retrieveFromDatabase(); Alert.alert('URI : ' + selectedImage + 'Note: ' +textToNote);Alert.alert('Note: ' +textToNote);}}  />
            </View>
        </View>
    )
